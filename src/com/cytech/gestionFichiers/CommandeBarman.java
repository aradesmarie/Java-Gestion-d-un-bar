...
public class CommandeBarman extends Application {
	public void start(Stage primaryStage) throws Exception {
		...
		Button b31 = new Button("Ajouter du Stock");
		...
		alerte.setTitle("Commande Barman");
		alerte.setHeaderText("Commande");
		alerte.setContentText("Appuyez sur OK pour saisir la commande");
		alerte.showAndWait();
			...
			public void handle(ActionEvent event) {
				...
				for (int i = 0; i < listeCocktails.size(); i++) {
					if (contenu.contentEquals(listeCocktails.get(i).getNom())) {
						...
						maListe.getItems().add("le prix total est de : " + listeCocktails.get(i).getPrix());
						System.out.println("le prix : " + listeCocktails.get(i).getPrix());
					}
				}
		...
		/* création d'une évènement sur b31 */
		b31.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event2) {
				/* création d'une nouvelle fenêtre */
				FlowPane rootp = new FlowPane();
				rootp.setPadding(new Insets(10));
				rootp.setVgap(5);
				rootp.setHgap(5);
				/* nouvelle liste */
				ListView<String> listeStock = new ListView<String>();
				listeStock.setOrientation(Orientation.VERTICAL);
				listeStock.setPrefSize(200, 100);
				listeStock.getItems().add("Stock");
				/* nouvelle zone de texte */
				TextField textFieldp = new TextField();
				/* création d'un nouveau bouton */
				Button bp = new Button("+");
				/* ajout des éléments à la nouvelle fenêtre */
				rootp.getChildren().add(textFieldp);
				rootp.getChildren().add(bp);
				rootp.getChildren().add(listeStock);

				/* action sur le bouton bp */
				bp.setOnAction(new EventHandler<ActionEvent>() {
					public void handle(ActionEvent eventp) {
						String contenup = textFieldp.getText();
						listeStock.getItems().add(contenup);
						/* pour les boissons non alcoolisees */
						for (int i = 0; i < lNA.size(); i++) {
							if (contenup.contentEquals(lNA.get(i).getNom())) {
								lNA.get(i).ajouterStock(1);
								System.out.println("le nouveau stock est de : " + lNA.get(i).getStock());
							}
						}
						/* pour les boissons alccolisees */
						for (int i = 0; i < lA.size(); i++) {
							if (contenup.contentEquals(lA.get(i).getNom())) {
								lA.get(i).ajouterStock(1);
								System.out.println("le nouveau stock est de : " + lA.get(i).getStock());
							}
						}
					}
				...
				Scene scenep = new Scene(rootp, 500, 300);
